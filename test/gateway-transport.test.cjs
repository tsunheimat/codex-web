...
test("stdio and SSH stdio initialize once and exchange structured JSON without a terminal", async (t) => {
...
test("SSH -W carries a WebSocket handshake to an existing runtime", async (t) => {
...
test("Unix socket attachment uses the WebSocket protocol and leaves the external listener alive", async (t) => {
